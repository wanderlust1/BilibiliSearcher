...
public class FlowLayout extends ViewGroup {
    ...
    /** 代表每一行的集合 */
    protected final List<Line> mLines = new ArrayList<>();
    ...
    private void requestLayoutInner() {
        new Handler(Looper.getMainLooper()).post(() -> requestLayout());
    }
    ...
    public void setAdapter(List<?> list, int res, ItemView mItemView) {
        ...
        removeAllViews();
        int layoutPadding = Tool.dp2px(getContext(), 10);
    ...
    public void setAdapter(List<?> list) {
        this.setAdapter(list, R.layout.tag_detail_text,
            (FlowLayout.ItemView<String>) (item, holder, inflate, position)
                -> holder.setText(R.id.tv_tag_text, item));
    }

    public interface ItemView<T> {
        void getCover(T item, ViewHolder holder, View inflate, int position);
    }
